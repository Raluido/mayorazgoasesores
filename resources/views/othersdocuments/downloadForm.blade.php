...
@section('content')
    <div class="d-flex justify-content-center mt-5">
        <div class="bg-light p-5 rounded border">
            <form method="POST" name="monthyearForm" action="/othersdocuments/download" enctype="multipart/form-data">
            ...
            <div class="mt-5">
                @if ($month || $year != null)
                    <a href="{{ url('/othersdocuments/download/' . $month . '/' . $year) }}"
                        class="btn btn-info">Descargar
                        modelo imputación de costes de
                        {{ ' ' . $month . ' ' . $year }}</a>
                    @role('contable')
                        <a href="{{ url('/othersdocuments/download/' . $month . '/' . $year) }}" class="btn btn-info">Descargar
